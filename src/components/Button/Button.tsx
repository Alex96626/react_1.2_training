import type { FC } from "react"
import { ProfileClass } from "../Profile/Profile.classname";
import "./Button.css";

type ButtonProps = {
    theme: 'primary' | 'edit' | 'back',
}

const Button:FC<ButtonProps> = ({theme}) => {
    const buttonContent = {
        'primary': 'Logout',
        'edit': (
            <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <g clipPath="url(#clip0_4123_303)">
                <path
                    d="M6.414 16.0001L16.556 5.85808L15.142 4.44408L5 14.5861V16.0001H6.414ZM7.243 18.0001H3V13.7571L14.435 2.32208C14.6225 2.13461 14.8768 2.0293 15.142 2.0293C15.4072 2.0293 15.6615 2.13461 15.849 2.32208L18.678 5.15108C18.8655 5.33861 18.9708 5.59292 18.9708 5.85808C18.9708 6.12325 18.8655 6.37756 18.678 6.56508L7.243 18.0001ZM3 20.0001H21V22.0001H3V20.0001Z"
                    fill="white"
                />
                </g>
                <defs>
                <clipPath id="clip0_4123_303">
                    <rect width="24" height="24" fill="white" />
                </clipPath>
                </defs>
            </svg>
        ),
        'back': (
              <svg
                xmlns='http://www.w3.org/2000/svg'
                width='8'
                height='14'
                viewBox='0 0 8 14'
                fill='none'
            >
                <path
                    d='M2.828 7.00023L7.778 11.9502L6.364 13.3642L0 7.00023L6.364 0.63623L7.778 2.05023L2.828 7.00023Z'
                    fill='#505050'
                />
            </svg>
        )
    }

    return (
        <button className={ProfileClass('Button', { theme })}>
            {buttonContent[theme]}
        </button>   
    )
}

export { Button }